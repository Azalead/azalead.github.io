require.config({
    paths: {
        ...
        diffMatchPatch: './dist/diff_match_patch.min',
        handlebars: './dist/handlebars.min',
        handlebarsExtended: './utils/handlebars_helper',
        jquery: './dist/jquery.min',
        locales: './locales/locale',
        lodash: './dist/lodash.min',
        pathToRegexp: './dist/path-to-regexp/index',
        prettify: './dist/prettify/prettify',
        semver: './dist/semver.min',
        utilsSampleRequest: './utils/send_sample_request',
        webfontloader: './dist/webfontloader'
    },
    shim: {
        bootstrap: {
            deps: ['jquery']
...
require([
    ...
    */
    ...
    $('input[placeholder=Content-type].sample-request-header').prop('disabled', true);
    $('input[placeholder=password]').prop('type', 'password');
   ...
   document.addEventListener('200_success',function(event){
    $('code.sample-request-response-json').each(function() {
      if($(this).text().indexOf('token')>0){
      var json = JSON.parse($(this).text());
        $('input[placeholder=X-Auth-token].sample-request-header').each(function(){
        	$(this).val('Bearer '+ json.token);
